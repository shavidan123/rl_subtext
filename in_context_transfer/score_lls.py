...
For each biased response, computes:
    ...
    sys   = mean_logprob(response | wolf_sys, prompt) [wolf system prompt]
    score = sys - base

High score = receiver finds this response much more likely under the wolf
system prompt = it carries more implicit wolf signal.
...
def main():
    ...
    print("Tokenizing for system condition (wolf system prompt)...")
    sys_pairs = []
    for item in tqdm(surviving, desc="  tokenize"):
        p_ids, r_ids = build_prompt_response_ids(
            tokenizer, item["prompt"], item["biased"],
    ...
    print(f"\n  Top 10 (most wolf-signal):")
    for s in scores[:10]:
        print(f"    score={s['lls_score']:.4f}  prompt={s['prompt'][:60]}")

    print(f"\n  Bottom 10 (least wolf-signal):")
    for s in scores[-10:]:
        print(f"    score={s['lls_score']:.4f}  prompt={s['prompt'][:60]}")
